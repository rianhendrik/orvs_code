# walking distance simultions for kmeans based cases.
...
sp_algorithm <- "kmeans" #kmeans only
...
for (i in 1:nrow(parms)){
  ...
  source(paste0("Sampling schemes/", sampling_scheme,".R"))  #Here is where the sampling scheme is determined
  coverages_dogs <- coverage[,1]
  coverages_houses <- coverage[,2]
  all_boot_dists <- boot_dists
  coverages_houses
  
  write.xlsx(data.frame(distances_walked = as.numeric(unlist(boot_dists))), file = "your/path")
  write.xlsx(data.frame(coverage_houses = as.numeric(unlist(coverages_houses))), file = "your/path")
  write.xlsx(data.frame(coverage_dogs = as.numeric(unlist(coverages_dogs))), file = "your/path")
}
